...
export type PlanSourceType = 'application' | 'academics' | 'manual' | 'capture' | 'learning' | 'external'
...
// §17 Insights history -- one entry per calendar day, accumulated locally on
// the device over time so Insights can show real trends instead of only
// live heuristics computed from the current snapshot of state.
...
// Cross-domain context signals pulled from Academics/Career/Learning state
// (see lib/contextEngine.ts) and fed into task scoring. This is the §14
// "one connected context engine" hook: Priority Engine stays deterministic,
// but its inputs now include state beyond the task itself.
export type ContextSignals = {
  attendanceRisk?: { courseName: string; pct: number; target: number; needToAttend: number } | null
  skillGap?: { skill: string; goalTitle: string } | null
  learningLoad?: { minutesThisWeek: number; tracks: number } | null
}
