...
function rowsForState(userId: string, state: any) {
  ...
  const plan = state.plan.map((block: any) => ({ id: block.id, user_id: userId, task_id: block.sourceType === 'capture' ? block.sourceId ?? null : null, title: block.title, day: block.day, start_time: block.time, duration_minutes: block.durationMinutes ?? 0, kind: block.kind, priority: block.priority, done: block.done, fixed: block.fixed, flexibility: block.flexibility, can_move: block.canMove, source_type: block.sourceType ?? null, source_id: block.sourceId ?? null, notes: block.notes ?? null }))
